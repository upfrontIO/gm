...
module.exports = function (proto) {
  ...
  proto.write = function write (name, callback) {
    ...
    this._preprocess(function (err) {
      console.log("Preprocess is done");
      console.log(err);
      if (err) return callback(err);
      console.log("Spawning from write");
  ...
  proto.stream = function stream (format, callback) {
    ...
    this._preprocess(function (err) {
      if (err) return callback(err);
      console.log("Spawning from stream");
  ...
  proto._exec = function _exec (args, callback) {
    console.log("Spawning from exec");
  ...
  proto._spawn = function _spawn (args, bufferOutput, callback) {
    ...
    // setTimeout(function(){
    //   console.log('Logging for process ' + proc.pid);
    //   exec('ps -p ' + proc.pid + ' -o %cpu,%mem', console.log);
    //   console.log('Child is connected? ' + proc.connected);
    // }, 1000);

    console.log(proc.pid + ": Timeout is:");
    console.log(timeout);
    if (timeout) {
      timeoutId = setTimeout(function(){
        console.log(proc.pid + ": Timeout reached")
      ...
      console.log(proc.pid + ": Set Timeout");
    }
    ...
    if (bufferOutput) {
      ...
      proc.on('close', onExit = function (code, signal) {
        console.log(proc.pid + ": In onClose");
      ...
      proc.on('error', function(err){
        console.log(proc.pid + ": In onError");
        if (err.code === 'ENOENT') {
    ...
    function cb (err, stdout, stderr, cmd) {
      console.log(proc.pid + ": In cb method");
      console.log(cb.called);
      if (cb.called) return;
      if (timeoutId) {
        console.log(proc.pid + ": Clearing Timeout");
      ...
      console.log(proc.pid + ": Calling final callback");
